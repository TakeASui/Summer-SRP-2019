...
def playNimWithCash1(n, d, e, remove, chance):
...
def playNimWithCash2(n, d, e, remove, chance):
    ...
    while(True):
        ...
        #determine the computer's move
        for x in remove:
            if x <= stones and x <= playerOneCash:
                possibleMoves.append(x)
        if random.uniform(0, 1) < chance:
            playerOneMove = moveTable[stones][playerTwoCash][playerOneCash]
        else:
            if len(possibleMoves) > 0:
                playerOneMove = random.choice(possibleMoves)
            else:
                playerOneMove = 0
...
while True:
    ...
    if restartOption == '3':
        ...
        invalidInput = True
        while(invalidInput):
            chance = input("How often would you like the computer to play perfectly (decimal between 0 and 1 inclusive): ")
            invalidInput = False
            if(not isNumber(chance) or float(chance) > 1 or float(chance) < 0):
                print("Input must be a decimal value between 0 and 1 inclusive")
                invalidInput = True
    ...
    if playerOption == '1':
        playNimWithCash1(stones, player1, player2, rule, chance)
    elif playerOption == '2':
        playNimWithCash2(stones, player1, player2, rule, chance)
        
    invalidInput = True
    while(invalidInput):
        restartOption = input("Enter (1) to keep the same rules, (2) to change starting cash, (3) to change everything, (nothing) to exit game: ")
        invalidInput = False
        if(restartOption != "1" and restartOption != "2" and restartOption != "3" and restartOption != ""):
